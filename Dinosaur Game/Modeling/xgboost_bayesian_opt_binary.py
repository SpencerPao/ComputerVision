...
def create_save_models(dtrain,
                       ...
                       processor_type,
                       number_of_classes):
    """Create the trained models using Bayesian Optimization."""
    ...
    # create search space ( can add more parameters as need be)
    search_space = [
...
def make_final_predictions(dcalib,
                           ...
                           type_error,
                           accepted_models_num):
    """Use to print out evaluation metrics (confusion matrix of the models)."""
    ...
    for i in range(num_models):
        # print(threshold_multiplier, threshold, threshold_multiplier*threshold)
        ...
        if bool1 and bool2:
            ...
            try:
                ...
                list_predicted_prob.append(np.round(xgb_model.predict(dprod)))
                if i == 0:
                    print("Y_prod", y_prod)
                    print("list_predicted_prob", list_predicted_prob[0])
...
if __name__ == '__main__':
    ...
    # read data
    x_calib = np.load(data_directory + 'screenshots_retry.npy')
    y_calib = np.load(data_directory + 'command_keys_retry.npy')
    ...
    if calculation_type == 'calibration':
        ...
        x_prod = np.load(data_directory + 'screenshots_retry.npy')
        y_prod = np.load(data_directory + 'command_keys_retry.npy')
